...
【注—关于minmaxL】h(x)=0, c(x)<=0，现在是取$$L(x,\alpha,\beta)$$的最大值，$$\alpha$$*c(x)是<=0，所以L(a,b,x)只有在$$\alpha$$*g(x) = 0的情况下才能取得最大值，否则，就不满足约束条件，因此$$max\limits_{\alpha,\beta}L(x,\alpha,\beta)$$在满足约束条件的情况下就是f(x)。
